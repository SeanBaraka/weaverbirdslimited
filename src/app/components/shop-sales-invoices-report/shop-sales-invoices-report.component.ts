...
import {ClearInvoiceComponent} from '../clear-invoice/clear-invoice.component';
...
export class ShopSalesInvoicesReportComponent implements OnInit {
  ...
  shop: any;
  constructor(
    private shopService: ShopManagerService,
    private dialog: MatDialog,
    private salesService: ProductSaleService) {
    shopService.getShopSaved().subscribe((shop) => {
      this.shop = shop;
    });
  }
      ...
      content: [
        ...
        },
        {
          table: {
            ...
            body: [
              ['Date', 'Receipt No.', 'Customer', 'Sub Total'],
              ...sales.map((p => (
                [
                  p.date, p.receiptNumber, p.customer.name,
  ...
  filterSalesControl(value: any): void {
    ...
    const invoices = this.shopInvoices.filter((x) => x.customer.name.toString().toLowerCase().includes(value.toLowerCase()));
  ...
  getSalesTotalAmount(sales: any[]): number {
    let invoiceSales = 0;
    sales.forEach((sale) => {
      invoiceSales += sale.saleAmount;
    });
    return invoiceSales;
  }

  clearInvoice(invoice: any): void {
    this.dialog.open(ClearInvoiceComponent, {
      width: '720px',
      height: '100vh',
      data: invoice
      ...
      content: [
        ...
        sale.paymentMethod === 'INVOICE' ? {
          columns: [
            [
              ...
              {
                text: `INVOICE NO:  #${sale.receiptNumber}`,
                style: 'textRegular',
                alignment: 'left'
              },
              {text: 'Pourtap Limited', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {text: 'P.O. Box 3305-90100 NAIROBI KENYA', style: 'textRegular', alignment: 'left'}
            ],
        ...
        },
        {
          table: {
            ...
            body: [
              [{text: 'Product Name', style: 'tableHeader'}, {text: 'QTY', style: 'tableHeader'}, {text: 'PRICE', style: 'tableHeader'}, {text: 'PRICE', style: 'tableHeader'}],
        ...
        },
        {
          columns: [
            [
              {
                text: 'Sub Total: ', style: 'textRegularLarge', bold: true, colspan: 6, margin: 0
              }
            ],
            [
              {
        ...
        },
        {
          columns: [
            [
              {
                text: 'TOTAL: ', style: 'textRegularLarge', bold: true, colspan: 6, margin: 0
              }
            ],
            [
              {
