...
export class MakePurchaseComponent implements OnInit {
   ...
   */
  changeQuantity(serial, value: any): void {
    const purchaseItem = this.purchaseItems.find(x => x.serialNumber == serial )
  ...
  removeItemByIndex(index: any): void {
    const item = this.purchaseItems.indexOf(index);
    this.purchaseItems.splice(item, 1)
  }
   ...
   */
  supplierSearch(supplierName: string): void {
    const supplier = this.suppliers.find(x =>x.name === supplierName);
    this.supplierSelected = true;
    this.currentSupplier = supplier
    this.productSearch() // get the list of products
  }

  productSearch(): void {
    this.stockService.getShopStock(this.shopData.shopId).subscribe((products) => {
          ...
          },
          {
            columns: [
              [
                ...
                {
                  text: `Purchase Order No:  #${this.poNumber}`,
                  style: 'textRegular',
                  alignment: 'left'
                },
              ...
              [
                ...
                {text: `SUPPLIER:  ${this.currentSupplier.name.toUpperCase()}`, style: 'textRegular', alignment: 'right'},
                {text: `TEL NO: ${this.currentSupplier.telephone}`, style: 'textRegular', alignment: 'right'}
              ]
            ]
          }          ,
          {
