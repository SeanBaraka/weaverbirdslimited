  ...
  printReport(productSales: any[]): void {
    const documentDefinition = {
      content: [
        ...
        {
          text: `Product Sales Report - ${this.shop.name}`,
          bold: true,
          style: 'subHeading'
        },
        {
          table: {
            ...
            body: [
              ['Product Id', 'Name ', 'Selling Price', 'Units Sold', 'Sub Total'],
              ...productSales.map((p => (
                [
                  p.id, p.name, p.sellingPrice, p.quantity,
                  (p.quantity * p.sellingPrice).toFixed(2)
                ]))),
              [
                {
                  text: 'Total', fontSize: 8, bold: true, colspan: 6
                }, {}, {}, {}, this.getSalesTotalAmount(productSales).toFixed(2)
              ]
            ]
          },
