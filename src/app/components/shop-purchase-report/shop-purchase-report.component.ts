...
@Component({
  selector: 'app-shop-purchase-report',
  templateUrl: './shop-purchase-report.component.html',
  styleUrls: ['./shop-purchase-report.component.sass']
})
export class ShopPurchaseReportComponent implements OnInit {

  shop: any;
  purchases: any[] = [];
  constructor(
    private shopService: StockDataService,
    private shopManage: ShopManagerService
  ) {
    shopManage.getShopSaved().subscribe((shop) => {
      this.shop = shop;
    });
  }
  ...
  getPurchases(): void {
    ...
    this.shopService.getShopPurchaseOrders(this.shop.id).subscribe((response: any[]) => {
      response.forEach((item) => {
        item.date = new Date(item.generatedAt).toLocaleDateString('en-GB');
      });
      this.purchases = response;
    });
  }

  reprintOrder(id: any): void {
    const po = this.purchases.find((item) => item.id === id);
        ...
        },
        {
          columns: [
            ...
            [
              ...
              {text: `SUPPLIER:  ${po.supplier.name.toUpperCase()}`, style: 'textRegular', alignment: 'right'},
              {text: `TEL NO: ${po.supplier.telephone}`, style: 'textRegular', alignment: 'right'}
            ]
          ]
        }          ,
        {
  ...
  updatePurchases($event: any[]): void {
    this.purchases = $event;
  }
}
