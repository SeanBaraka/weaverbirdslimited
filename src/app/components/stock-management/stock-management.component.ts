...
import { MessageNotificationsService } from 'src/app/services/message-notifications.service';
...
export class StockManagementComponent implements OnInit {
  ...
  closeShop(): void {
    const documentDefinition = {
      content: [
        ...
        {
          text: `Closing stock Report - Shop (${this.shopName})`,
          fontSize: 12,
          style: 'sectionHeader'
        },
        {
    ...
    this.stockData.closeShop(this.shopId, this.stockProducts, openingStock, closingStock, addedStock).subscribe((resp) => {
      if (resp) {
        // this.finished = true;
        const message = {
  ...
  getOpeningStock(): number {
    ...
    this.stockProducts.forEach((item) => {
      openingStockAmounts.push((item.quantity * item.sellingPrice));
    });
  ...
  getStockTotalAmount(): number {
    const totalAmount = [];
    this.stockProducts.forEach((item) => {
      totalAmount.push(item.sellingPrice * item.quantity);
    });
  ...
  getAllSales(): void {
    this.salesService.getSales(this.selectedShop.id).subscribe((response) => {
      if (response) {
  ...
  printInventory(detailed: boolean): void {
    const documentDefinition = {
      content: [
        ...
        {
          text: `Product Inventory Report - ${this.selectedShop.name}`,
          bold: true,
          style: 'subHeading'
        },
        {
          table: {
            headerRows: 1,
            widths: [150, detailed === true ? 'auto' : 0, 'auto', detailed === true ? 'auto' : 0],
            heights: (row) => {
              if (row === 0) {
                return 0;
              } else {
            ...
            body: [
              ['Product Name', detailed === true ? 'Units' : '', '@', detailed === true ? 'Sub Total' : ''],
              ...this.stockProducts.map((p => (
                [
                  p.name, detailed === true ? p.quantity : '', p.sellingPrice,
                  detailed === true ? (p.quantity * p.sellingPrice).toFixed(2) : ''
                ]))),
              [
                {
