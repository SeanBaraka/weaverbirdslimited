    ...
    const documentDefinition = {
      content: [
        ...
        },
        {
          table: {
            headerRows: 1,
            widths: [150, detailed === true ? 'auto' : 0, detailed === true ? 'auto' : 100, 'auto', detailed === true ? 'auto' : 150],
            heights: (row) => {
              if (row === 0) {
                return 0;
              } else {
            ...
            body: [
              ['Product Name', detailed === true ? 'Quantity' : '', detailed === true ? 'Cost Price' : 'Wholesale Price', 'Selling Price', detailed === true ? 'Sub Total' : ''],
              ...this.stockProducts.map((p => (
                [
                  p.name, detailed === true ? p.quantity : '', detailed === true ? p.costPrice : p.minPrice,  p.sellingPrice,
                  detailed === true ? (p.quantity * p.costPrice).toFixed(2) : ''
                ]))),
              [
                {
