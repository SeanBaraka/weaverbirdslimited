...
export class ShopSalesReportAllComponent implements OnInit {
  ...
  computeCartTotal(cartItems: any[], sale: any): number {
    if (cartItems != null && cartItems.length > 0) {
      let subTotals = [];
      cartItems.forEach(e => {
        const itemTotal = sale.paymentMethod === 'INVOICE' ? e.quantity * e.minPrice : e.quantity * e.sellingPrice;
  ...
  reprintReceipt(receiptId: number): void {
    const sale = this.sales.find(x => x.id === receiptId);
    ...
    const documentDefinition = {
      ...
      content: [
        {
          ...
          width: sale.paymentMethod === 'INVOICE' ? 100 : 400,
          alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'
        },
        sale.paymentMethod === 'INVOICE' ? {
          columns: [
            [
              { text: `Date - ${new Date(Date.now()).toLocaleDateString()}`, width: '*', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {
                text: sale.paymentMethod === 'INVOICE' ? `INVOICE NO:  #${sale.receiptNumber}` : `Receipt Number #${sale.receiptNumber}`,
                style: 'textRegular',
                alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'
              },
              {text: 'Pourtap Limited', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {text: `${this.shop.name}`, style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {text: 'P.O. Box 6408-00200 NAIROBI KENYA', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'}
            ],
            ...
            [
              ...
              {text: `CUSTOMER:  ${sale.customer.name}`, style: 'textRegular', alignment: 'right'},
              {text: `TEL NO: ${sale.customer.telephone}`, style: 'textRegular', alignment: 'right'}
            ]
          ]
        } : {
          columns: [
            [
              {text: 'Pourtap Limited', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {text: 'P.O. BOX 6782 - 00200 Nairobi, Kenya', style: 'textRegular', alignment: 'center'},
              {text: `${this.shop.name}`, style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {text: 'TELEPHONE: 0712694934 . 0722794988', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
              {text: 'Email Address: info@pourtaplimited.co.ke', style: 'textRegular', alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'},
        ...
        sale !== true ? {
          columns: [
            [
              ...
              {text: 'PIN Number. P051820673J', style: 'subHeading', alignment: 'center' },
              {text: `Receipt Number #${sale.receiptNumber} | Till Number: 5060027`, style: 'subHeading', bold: true, alignment: 'center' },
        ...
        {
          text: sale.paymentMethod === 'INVOICE' ? 'INVOICE SALE' : '',
          bold: true,
          style: 'subHeading',
          alignment: sale.paymentMethod === 'INVOICE' ? 'left' : 'center'
        },
        {
          ...
          table: {
            headerRows: 1,
            widths: [ sale.paymentMethod === 'INVOICE' ? '*' : 400, 'auto', 'auto', 'auto'],
            heights: (row) => {
              if (row === 0) {
                return 0;
              } else {
            ...
            body: [
              [{text:sale.paymentMethod === 'INVOICE' ? 'Product Name' : 'ITEM', style: 'tableHeader'}, {text: 'QTY', style: 'tableHeader'}, {text: 'PRICE', style: 'tableHeader'}, {text: 'PRICE', style: 'tableHeader'}],
              ...JSON.parse(sale.saleItems).map((p => (
                [
                  {text: p.name, style: 'textRegular', margin: sale.paymentMethod === 'INVOICE' ? [0,5,0,5] : [0, 10, 0, 10]},
                  {text: p.quantity, style: 'textRegular', margin: sale.paymentMethod === 'INVOICE' ? [0,5,0,5] : [0, 10, 0, 10]},
                  {text: p.sellingPrice, style: 'textRegular', margin: sale.paymentMethod === 'INVOICE' ? [0,5,0,5] : [0, 10, 0, 10]},
                  {text: (p.quantity * p.sellingPrice).toFixed(2), style: 'textRegular', margin: sale.paymentMethod === 'INVOICE' ? [0,5,0,5] : [0, 10, 0, 10]}
                ]))),
            ]
          },
          // layout: 'headerLineOnly',
          fontSize: sale.paymentMethod === 'INVOICE' ? 8 : 24
        },
        sale.paymentMethod === 'INVOICE' ? {
          text: '',
          margin: 0
        ...
        },
        {
          columns: [
              [
                {
                  text: 'Sub Total: ', style: 'textRegularLarge', bold: true, colspan: 6, margin: sale.paymentMethod === 'INVOICE' ? 0 : [0 , 35, 0, 35]
                }
              ],
              [
                {
                  text: `${this.computeCartTotal(JSON.parse(sale.saleItems), sale).toFixed(2)}`,
                  style: 'textRegularLarge',
                  bold: true,
                  alignment: 'right',
                  margin: sale.paymentMethod === 'INVOICE' ? [0, 0, 5, 0] : [0, 35, 0, 35]
                }
              ]
          ],
        },
        {
          columns: [
              [
                {
                  text: 'TAX: ', style: 'textRegularLarge', bold: true, colspan: 6, margin: sale.paymentMethod === 'INVOICE' ? 0 : [0 , 35, 0, 35]
                }
              ],
              [
                {
                  text: `${(sale.saleAmount * 0.16).toFixed(2)}`,
                  style: 'textRegularLarge',
                  bold: true,
                  alignment: 'right',
                  margin: sale.paymentMethod === 'INVOICE' ? [0, 0, 5, 0] : [0, 35, 0, 35]
        ...
        },
        {
          columns: [
              [
                {
                  text: 'TOTAL: ', style: 'textRegularLarge', bold: true, colspan: 6, margin: sale.paymentMethod === 'INVOICE' ? 0 : [0 , 35, 0, 35]
                }
              ],
              [
                {
                  text: `${sale.saleAmount.toFixed(2)}`,
                  style: 'textRegularLarge',
                  bold: true,
                  alignment: 'right',
                  margin: sale.paymentMethod === 'INVOICE' ? [0, 0, 5, 0] : [0, 35, 0, 35]
        ...
        },
        {
          columns: [
            {
              text: sale.paymentMethod === 'MOBILE' ? 'Amount paid' : sale.paymentMethod === 'INVOICE' ? '' : 'CASH PAID: ',
              style: 'textRegularLarge',
              margin: [0, 0, 0, 35],
              bold: true,
              colspan: 6
            }, {}, {
              text:  sale.paymentMethod === 'INVOICE' ? '' : `${sale.saleAmount.toFixed(2)}`,
              style: 'textRegularLarge',
              bold: true
            }
          ]
        ...
        sale.paymentMethod === 'INVOICE' != true ? {
      ...
      styles: {
        ...
        subHeading: {
          margin: [0, 20, 0, 10],
          fontSize: sale.paymentMethod === 'INVOICE' ? 12 : 26
        },
        textRegular: {
          margin: [0, 5, 0, 5],
          fontSize: sale.paymentMethod === 'INVOICE' ? 10 : 24
        },
        textRegularLarge: {
          margin: [0, 5, 0, 5],
          fontSize: sale.paymentMethod === 'INVOICE' ? 10 : 26
        },
        tableHeader: {
          bold: true,
          margin: [0, 5, 0, 5]
  ...
  printSalesReport(sales: any[]): void {
    const detailed = true;
    const documentDefinition = {
      content: [
        {
        ...
        {
          text: `Sales Report - ${this.shop.name}`,
          bold: true,
          style: 'subHeading'
        },
        {
          table: {
            ...
            body: [
              ['Date', 'Receipt No.', 'Mode of Payment', 'Sub Total'],
              ...sales.map((p => (
                [
                  p.date, p.receiptNumber, p.paymentMethod,
                  p.saleAmount.toFixed(2)
                ]))),
              [
                {
                  text: 'Total', fontSize: 8, bold: true, colspan: 6
                }, {}, {}, this.getSalesTotalAmount().toFixed(2)
              ]
            ]
          },
          layout: 'headerLineOnly',
    ...
    pdfMake.createPdf(documentDefinition).print({silent: false}, win); // print the table data
    // win.close();
  }

  private getSalesTotalAmount(): number {
    let totalSales = 0;
